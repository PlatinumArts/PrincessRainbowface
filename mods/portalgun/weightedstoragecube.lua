...
on_place=function(itemstack, user, pointed_thing)
	if pointed_thing.type=="node" then
		portalgun.new=1
		local m=minetest.env:add_entity(pointed_thing.above, "portalgun:wsc"..ii)
...
minetest.register_node("portalgun:wscspawner2_" .. ii, {
	...
	mesecons = {effector = {
		action_on = function (pos, node)
			...
			portalgun.new=1
			local m=minetest.env:add_entity(pv, "portalgun:wsc" ..ii)
			m:setacceleration({x=0, y=-10, z=0})
		end
	}}
})
...
on_step= function(self, dtime)
		...
		if self.timer2>10 then
			self.timer2=0
			for i, ob in pairs(minetest.get_objects_inside_radius(self.object:getpos(), 20)) do
...
minetest.register_node("portalgun:wscspawner"..ii, {
	...
	on_timer = function (pos, elapsed)
		...
		portalgun.new=1
		local m=minetest.env:add_entity(pos, "portalgun:wsc" ..ii)
		m:setacceleration({x=0,y=-10,z=0})
		return true
	end,
})
