		...
		for ii, ob in pairs(minetest.get_objects_inside_radius(pos1, self.area)) do
			if pos2~=0 then
				...
				if ob:is_player() then
					...
					m:setvelocity(v)
					m:setacceleration({x=0,y=-10,z=0})
					ob=m
...
-- the project
	...
	for i=1, portalgun_max_rage,0.5 do
		...
		if minetest.registered_nodes[nname].walkable then
			...
			if minetest.registered_nodes[minetest.get_node({x=rnd(pos.x+(dir.x*i)), y=rnd(pos.y+(dir.y*i)+1), z=rnd(pos.z+(dir.z*i))}).name].walkable==false and rnd(user:getpos().y)>rnd(pos.y+(dir.y*i)) then
				portalgun_setportal(pos,name,dir,i,mode,"y+")
				return itemstack
			elseif minetest.registered_nodes[minetest.get_node({x=rnd(pos.x+(dir.x*i)), y=rnd(pos.y+(dir.y*i)-1), z=rnd(pos.z+(dir.z*i))}).name].walkable==false and rnd(user:getpos().y)<rnd(pos.y+(dir.y*i)) then
				portalgun_setportal(pos,name,dir,i,mode,"y-")
				return itemstack
			elseif minetest.registered_nodes[minetest.get_node({x=rnd(pos.x+(dir.x*i)-1), y=rnd(pos.y+(dir.y*i)), z=rnd(pos.z+(dir.z*i))}).name].walkable==false  and rnd(user:getpos().x)<rnd(pos.x+(dir.x*i)) then
				portalgun_setportal(pos,name,dir,i,mode,"x-")
				return itemstack
			elseif minetest.registered_nodes[minetest.get_node({x=rnd(pos.x+(dir.x*i))+1, y=rnd(pos.y+(dir.y*i)), z=rnd(pos.z+(dir.z*i))}).name].walkable==false and rnd(user:getpos().x)>rnd(pos.x+(dir.x*i)) then
				portalgun_setportal(pos,name,dir,i,mode,"x+")
				return itemstack
			elseif minetest.registered_nodes[minetest.get_node({x=rnd(pos.x+(dir.x*i)), y=rnd(pos.y+(dir.y*i)), z=rnd(pos.z+(dir.z*i)-1)}).name].walkable==false and rnd(user:getpos().z)<rnd(pos.z+(dir.z*i)) then
