...
function line_of_sight_water(self, pos1, pos2, stepsize)
	...
	-- normal walking and flying lovemobs can see you through air
	if s == true then
		return true
	end

	-- swimming lovemobs can see you through water
	if s == false
	and self.fly
		...
		if self.attack_type == "explode" then
			...
			if dist > self.reach then
				...
				if self.timer > 3 then
					...
					-- hurt player/lovemobs caught in blast area
					...
					lovemobs:explosion(pos, radius, 0, 1, self.sounds.explode)
...
local mob_punch = function(self, hitter, tflp, tool_capabilities, dir)
	...
	-- attack puncher and call other lovemobs for help
...
lovemobs.spawning_mobs = {}

-- register mob function
function lovemobs:register_mob(name, def)

	lovemobs.spawning_mobs[name] = true
...
end -- END lovemobs:register_mob function

-- global functions

function lovemobs:spawn_specific(name, nodes, neighbors, min_light, max_light,
	...
	if new_chance ~= nil then
		...
		if new_chance == 0 then
			print("[lovemobs Redo] " .. name .. " has spawning disabled")
			return
		end

		chance = new_chance

		print ("[lovemobs Redo] Chance setting for " .. name .. " is now " .. chance)
	...
	minetest.register_abm({
		...
		action = function(pos, node, aoc, active_object_count_wider)
			...
			or not lovemobs.spawning_mobs[name] then
				return
			end
			...
			-- lovemobs cannot spawn in protected areas when enabled
...
--				print ("[lovemobs] Spawned " .. name .. " at "
--				.. minetest.pos_to_string(pos) .. " on "
--				.. node.name .. " near " .. neighbors[1])
			else
				print ("[lovemobs]" .. name .. " failed to spawn at "
...
function lovemobs:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height, day_toggle)

	lovemobs:spawn_specific(name, nodes, {"air"}, min_light, max_light, 30,
...
function lovemobs:explosion(pos, radius, fire, smoke, sound)
...
function lovemobs:register_arrow(name, def)
...
-- Spawn Egg
function lovemobs:register_egg(mob, desc, background, addegg, no_creative)
...
function lovemobs:capture_mob(self, clicker, chance_hand, chance_net, chance_lasso, force_take, replacewith)
	...
	and clicker:get_inventory() then
		...
		if clicker:get_inventory():room_for_item("main", mobname) then
			...
			elseif tool:get_name() == "lovemobs:net" then
			...
			elseif tool:get_name() == "lovemobs:magic_lasso" then
...
function lovemobs:feed_tame(self, clicker, feed_count, breed, tame)
	...
	if item:get_name() == "lovemobs:nametag"
